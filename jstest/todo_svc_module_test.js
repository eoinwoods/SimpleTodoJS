...
test("test initialisation results in 5 items", function() {
	...
	deepEqual(items.length, 5, "Expected 5 items in initial TODO list")
}) ;

test("test adding an item", function() {
	todosvc.reset() ;
...
test("test removing an item", function() {
	todosvc.reset() ;
	var items = todosvc.get_todo_items(1) ;
